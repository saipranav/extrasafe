...
Hasher = {
    ...
    hashy: function() {
        var toHashString = Hasher.siteTag + Hasher.masterPassword + Hasher.extraSecuritySequence;
        Hasher.password = CryptoJS.SHA3(toHashString).toString();
    },
    ...
    checker: function() {
        ...
        // replaces the password at replaceIndex, with character taken from predefined array
        // (take the password character at position (take from position array ) convert to integer value)
        // (fold the integer with predefined array length so that it stays within predefined array)
        ...
        if (/^[a-z]*$/.test(Hasher.password) == true || Hasher.numberPos.length < 4) {
            // Why 3 times?
            for (var i = 0; i < 3; i++) {
                var replaceIndex = Hasher.getReplaceIndex(Hasher.smallCharactersPos, i);
                var indexToUse = ((Hasher.password[Hasher.smallCharactersPos[i]]).charCodeAt(0))%(Hasher.numbers.length-1);
                Hasher.password = Hasher.replaceAt(replaceIndex, Hasher.numbers[indexToUse]);
        ...
        if (/^[0-9]*$/.test(Hasher.password) == true || Hasher.smallCharactersPos.length < 4) {
            // Why 3 times?
            for (var i = 0; i < 3; i++) {
                var replaceIndex = Hasher.getReplaceIndex(Hasher.numberPos, i);
                var indexToUse = ((Hasher.password[Hasher.numberPos[i]]).charCodeAt(0))%(Hasher.smallCharacters.length-1);
                Hasher.password = Hasher.replaceAt(replaceIndex, Hasher.smallCharacters[indexToUse]);
        ...
        if (/^([a-z]|(@|!|&|#|\*)|[0-9])*$/.test(Hasher.password) == true) {
            ...
            var indexToUse = ((Hasher.password[Hasher.smallCharactersPos[Hasher.smallCharactersPos.length-1]]).charCodeAt(0))%(Hasher.capitalCharacters.length-1);
            Hasher.password = Hasher.replaceAt(replaceIndex, Hasher.capitalCharacters[indexToUse]);
            Hasher.injectedChars.push(replaceIndex);

            replaceIndex = Hasher.getReplaceIndex(Hasher.smallCharactersPos, 1);
            var indexToUse = ((Hasher.password[Hasher.smallCharactersPos[Hasher.smallCharactersPos.length-2]]).charCodeAt(0))%(Hasher.capitalCharacters.length-1);
            Hasher.password = Hasher.replaceAt(replaceIndex, Hasher.capitalCharacters[indexToUse]);
            Hasher.injectedChars.push(replaceIndex);
        }
        ...
        if (/^([a-z]|[A-Z]|[0-9])*$/.test(Hasher.password) == true) {
            ...
            var indexToUse = ((Hasher.password[Hasher.numberPos[Hasher.numberPos.length-1]]).charCodeAt(0))%(Hasher.specialCharacters.length-1);
            Hasher.password = Hasher.replaceAt(replaceIndex, Hasher.specialCharacters[indexToUse]);
            Hasher.injectedChars.push(replaceIndex);
        }
    },
    // takes the passwords char code (int) at charAt, fold it to the size of array/2,
    // to make sure index stays inside the password (because array elements are subset of password indexes)
    // it takes care of injected characters not being replaced
    // it might some times give undefined which makes the password grow by repeating chars at replaceAt function,
    // but finalTrim takes care of this issue
    ...
    fillCountCharacters: function() {
        // reset array to 0
        Hasher.injectedChars.length = Hasher.specialCharactersPos.length = 0;
        Hasher.numberPos.length = Hasher.smallCharactersPos.length = Hasher.capitalCharactersPos.length = 0;
